...
export function registerAppListener(navigation){
  FCM.on(FCMEvent.Notification,  async(notif) => {
    ...
    console.log("Notificationa", notif);
    if (notif.fcm.body ==undefined)
    {
      return;
    } 

    var person_info = JSON.parse(notif.person_info);
    var fullname = person_info['fullname']
    var channel = notif.channel;
    var group = JSON.parse(notif.groups)[0]['name'];
  
    var notData ={
      sentTime: notif["google.sent_time"],
      content: notif.fcm.body,
      content: group +" " + fullname + ' arrives '  + channel,
      checkStatus : false,
      snapshot:notif.snapshot
    }
    
    StorageHepler.getStorage('notificationDataList').then(_data => {

      var temp = _data.NotificationList;
      temp.push(notData)
    ...
    .catch(error=> {
      StorageHepler.setStorage("notificationDataList", {
        NotificationList: [notData]
      })
      navigation.navigate('notification')

    });

   // AsyncStorage.setItem('notificationDataArray',tmp,()=>{
